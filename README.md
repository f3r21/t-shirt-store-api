...
| Promo codes, Optional Feature 13, the manager's half | Done, three operations: a manager creates a code, reads one page of codes with the number of orders each has been used on, and disables or enables one. The code column is `citext`, so `SAVE10` and `save10` are one code and the second create answers 409. One column carries the discount: a percentage stops at 100 and a fixed amount is in minor units, and `discountType` says which. A client applying a code at checkout is the other half, and it is not written yet |
