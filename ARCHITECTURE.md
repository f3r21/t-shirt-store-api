...
**Broken access control is first, and thinnest.** Authorization is one guard,
`src/auth/roles.guard.ts`, bound per controller while the token guard is global
(`src/auth/auth.module.ts:39`), so both rely on Nest running global guards first and nothing asserts
it. It also fails open: a handler with no `@Roles` passes through (`roles.guard.ts:38`), so a
forgotten decorator is silent. CASL, which the challenge requires, is a dependency at
`package.json:35` that no file in `src` imports.

**Authentication is second, with two narrow holes.** `argon2.hash` is called with no options
...
**The rate limit is third, and the deploy shape breaks it.** The throttler has no storage adapter,
so its counter lives in one process's memory and five attempts per window becomes five *per
replica*. Reset-password goes first: a distributed attacker gets N times the budget for nothing. The
fix is the Redis storage adapter, which is also the reason Redis sits in the compose file already.
Webhook replay belongs in this list and is not in it, because the webhook is unwritten.
...
**One regression would reach production unnoticed today.** Delete `@Roles('manager')` from
